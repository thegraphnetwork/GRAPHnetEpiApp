...
cleaning_run <- function(country, df) {
  ...
  #path_name  <- here("data/kenya")
  
  #file <- list.files(path_name)
  source(here("Scripts", "anonymisation.R"))
  df <- df
  raw <- anonym(country, df)
  ...
  source(here("maps", glue::glue("map_{country}.R")))
  ...
  # Save raw csv file for archiving
  #(raw_csv_name <- here("data/archive", glue("{country}_{latest_report_date}_raw.csv")))
  #write.csv2(raw,raw_csv_name,row.names = F) # write.csv2 defaults to sep = ";"
  
  # Save clean csv file for archiving
  #(clean_csv_archive_name <- here("data/archive", glue("{country}_{latest_report_date}_clean.csv")))
  #write.csv2(clean,clean_csv_archive_name,row.names = F)  # write.csv2 defaults to sep = ";"
