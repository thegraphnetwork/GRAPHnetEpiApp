...
clean_columns[['report_date']] =  clean_numeric_dates('report_date')
...
clean_columns[['patinfo_ageonset_years']] = function(df) ifelse(df[,'Age_Group'] == "Unknown", NA , df[,'Age_Group'])
columns_used = append(columns_used,"Age_Group")
...
clean_columns[['patinfo_occus']] = function(df) ifelse(grepl("Infirmière|Hospital|Médecin|docteur|santé",df$patinfo_occus), "Y",
...
clean_columns[['pat_symptomatic']] = function(df) ifelse(tolower(df[,'patsympt_fever'])=="yes" |tolower(df[,'patsympt_sorethroat'])=="yes"|tolower(df[,'patsympt_cough'])=="yes"|tolower(df[,'patsympt_runnynose'])=="yes"|tolower(df[,'patsympt_short'])=="yes"|tolower(df[,'patsympt_other'])!="no" & !is.na(df[,'patsympt_other']) & tolower(df[,'patsympt_other'])!="don't know","Y",ifelse(df[,'patsympt_fever']=="No" &df[,'patsympt_sorethroat']=="No" & df[,'patsympt_cough']=="No" & df[,'patsympt_runnynose']=="No" & df[,'patsympt_short']=="No" & df[,'patsympt_other']=="No","N",NA))
...
clean_columns[['patcourse_dateonset']] = clean_numeric_dates('patcourse_dateonset')
...
clean_columns[['patcourse_datedeath']] = clean_numeric_dates('patcourse_datedeath')
...
clean_columns[['patinfo_resadmin4']] = map_to('patinfo_resadmin42')
columns_used = append(columns_used,"patinfo_resadmin42")
...
clean_columns[['consultation_dateHF']] = clean_numeric_dates('consultation_dateHF')
...
clean_columns[['expo_other']] = function(df) ifelse(is.na(df[,'expo_other']) | df[, 'expo_other'] == "Non", NA, df[, 'expo_other'])
...
clean_columns[['Lab_datetaken']] = clean_numeric_dates('Lab_datetaken')
...
clean_columns[['patcourse_datedischarge']] = clean_numeric_dates('patcourse_datedischarge')
...
columns_unused = list("patinfo_ageonset")
